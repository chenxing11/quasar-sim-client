    ...
    <div class="q-pa-xs row">
      ...
      <q-list class="q-py-xs q-px-md row col-12">
        ...
        <q-item class="col-12 print-header">
          ...
          <q-item-section class="list-title col-6">订单号: 永续订单</q-item-section>
        </q-item>
        <q-item class="col-12 print-header">
      ...
      this.$api.Print.P_getShipmentById(id, token).then(res => {
        if (res.data.code === 0) {
          ...
          this.$api.Print.P_getSupLoc(
            this.shipment.supplier.id,
            this.shipment.toLocation.id,
            token
          ).then(res => {
          ...
          this.orderRcved = res.data.data.shipmentLines.map(val => {
            ...
            json.unitCostVat = parseFloat(
              val.unitCost * (1 + val.costVatRate / 100)
            ).toFixed(2)
            json.qtyCheck = val.qtyExpected - val.qtyReceived
            return json
          })
          this.rcvorder = this._.unionBy(
            this.orderRcved,
            this.orderList,
            'itemId'
          )
          this.ordersItem = this.rcvorder.map(function (item) {
            return item['itemId']
          })
