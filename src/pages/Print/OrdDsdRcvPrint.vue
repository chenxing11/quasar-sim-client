<template>
  ...
  >
    <div class="q-pa-xs row">
      <h6 class="col-12 text-center print-title">北京华联综合超市股份有限公司<br>验货清单（永续订单收货）</h6>
      <q-list class="q-py-xs q-px-md row col-12">
        <q-item class="col-12 print-header">
          <q-item-section class="list-title col-6">收货门店: {{shipment.toLocation.id + ' ' + shipment.toLocation.name}}</q-item-section>
          <q-item-section class="list-title col-6">收货号: {{ shipment.id }}</q-item-section>
        </q-item>
        <q-item class="col-12 print-header">
          <q-item-section class="list-title col-6">供应商编号: {{shipment.supplier.id }}</q-item-section>
          <q-item-section class="list-title col-6">订单号: {{ shipment.order.externalNo }}</q-item-section>
        </q-item>
        <q-item class="col-12 print-header">
          <q-item-section class="list-title col-6">供应商名称: {{shipment.supplier.name }}</q-item-section>
          <q-item-section class="list-title col-6">到货日期: {{ shipment.receivedDate.substr(0, 10) }}</q-item-section>
        </q-item>
        <q-item class="col-12 print-header">
          <q-item-section class="list-title col-6">供应商地址: {{shipment.supplier.address }}</q-item-section>
          <q-item-section class="list-title col-6">购货单位: {{ supplier.settlementCenter === null ? '' : supplier.settlementCenter.description}}</q-item-section>
        </q-item>
        <q-item class="col-12 print-header">
          <q-item-section class="list-title col-6">供应商电话: {{shipment.supplier.contactMobile }}</q-item-section>
    ...
    getlist (id, token) {
      ...
      this.$api.Print.P_getShipmentById(id, token).then(res => {
        if (res.data.code === 0) {
          ...
          this.$api.Print.P_getSupLoc(this.shipment.supplier.id, this.shipment.toLocation.id, token).then(res => {
          ...
          this.orderRcved = res.data.data.shipmentLines.map(val => {
            ...
            json.unitCost = parseFloat(val.unitCost).toFixed(2)
            json.unitCostVat = parseFloat(val.unitCost * (1 + val.costVatRate / 100)).toFixed(2)
