<template>
  ...
  >
    <div class="q-pa-xs row">
      <h5 class="col-12 text-center print-title">
        北京华联综合超市股份有限公司
        <br />直送退货清单
      </h5>
      <q-list class="q-py-xs q-px-md row">
        <q-item class="col-12 print-header">
          <q-item-section class="list-title col-6">退货门店: {{Rtv.location.id + ' ' + Rtv.location.name}}</q-item-section>
          <q-item-section class="list-title col-6">退货号: {{ shipment.id }}</q-item-section>
        </q-item>
        <q-item class="col-12 print-header">
          <q-item-section class="list-title col-6">供应商编号: {{Rtv.supplier.id }}</q-item-section>
          <q-item-section class="list-title col-6">外部单据号: {{ Rtv.externalNo }}</q-item-section>
        </q-item>
        <q-item class="col-12 print-header">
          <q-item-section class="list-title col-6">供应商名称: {{Rtv.supplier.name }}</q-item-section>
          <q-item-section class="list-title col-6">退货日期: {{ shipment.shippedDate.substr(0, 10) }}</q-item-section>
        </q-item>
        <q-item class="col-12 print-header">
          <q-item-section class="list-title col-6">供应商地址: {{Rtv.supplier.address }}</q-item-section>
        ...
        <q-item class="col-12 print-header">
          <q-item-section class="list-title col-6">供应商电话: {{Rtv.supplier.contactMobile }}</q-item-section>
          <q-item-section class="list-title col-6">收货部电话: {{shipment.fromLocation.managerPhone}}</q-item-section>
        </q-item>
        <q-item class="col-12 print-header">
...
export default {
  ...
  methods: {
    ...
    getlist (id, token) {
      ...
      this.$api.Print.P_getRtv(id, token).then(res => {
        if (res.data.code === 0) {
          this.POP_MSG(res.data.msg)
        } else {
          this.Rtv = res.data.data
          this.$api.Print.P_getSupLoc(
            this.Rtv.supplier.id,
            this.Rtv.locationId,
            token
          ).then(res => {
            this.supplier = res.data.data
          })
          this.$api.Print.P_GetShipmentByReturnId(this.Rtv.id, token).then(
            res => {
