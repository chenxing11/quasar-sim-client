          ...
          <template v-slot:item="props">
            ...
            <q-card>
                <q-card-section horizontal>
                  ...
                  >
                    <div><span class="q-pa-xs">库存调整单号</span><span class="q-pa-xs">{{ props.row.id }}</span></div>
...
export default {
  ...
  watch: {
    ...
    //     this.createStock(this.inventoryAdjustId, this.tempSelected[0].createdDate)
    //   }
    // }
  },
  created () {
  ...
  methods: {
    ...
    get_order (id, createdDate) {
      this.$router.push({ path: '/Inv/CreateInvAdjust', query: { inventoryAdjustId: id, createdDate: createdDate } })
    },
    getList (LocationId) {
      this.$api.Inventory.ListInventoryAdjustInProcByLoc(LocationId).then(res => {
        console.log(res)
        if (res.data.code > 0) {
          this.temp = false
          this.InventoryList = res.data.data.map(val => {
            let json = {}
            json.id = val.id
