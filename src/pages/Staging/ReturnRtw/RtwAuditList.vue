...
export default {
  ...
  watch: {
    tempSelected (val) {
      if (this.tempSelected[0]) {
        this.rtwId = this.tempSelected[0].id
        this.get_order(this.rtwId)
      }
    }
  },
  created () {
  ...
  methods: {
    ...
    get_order (id) {
      this.$router.push({ path: '/ReturnRtw/RtwAuditOrder', query: { rtwId: id } })
    },
    getList (LocationId) {
      this.$api.Rtw.ListReturnToWhInSubmittedByLoc(LocationId).then(res => {
        console.log(res)
        if (res.data.code > 0) {
          this.temp = false
