...
<template>
  <q-page>
    ...
    <div class="flex flex-center q-px-md">
      <q-form @keydown.enter="get_order(orderNo)" @submit="RETURNFALSE()" class="orderinput">
      <q-input autofocus
               outlined
    ...
    <div class="column items-center">
      ...
      > 无进行中的收货单</div>
      <q-table v-if='!temp' class=" self-center col"
               title="订单收货暂存单"
...
export default {
  ...
  methods: {
    ...
    get_rcved (id) {
      this.$api.Rcv.getPoDsdListShipment(id).then(res => {
        if (res.data.code === 0) {
          this.temp = true
        ...
        this.$api.Rcv.getOrder(val).then(res => { // 获取订单信息
          if (res.data.code === 0) {
            ...
            if (res.data.data.orderLines[0].locationId !== this.$store.state.location.id) {
              ...
              this.$router.push({ path: '/Rcv/OrdDsd', query: { orderNo: val, orderId: this.orderId, loc: this.$store.state.location.id } })
            }
          }
        })
      }
