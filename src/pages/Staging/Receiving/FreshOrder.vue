  ...
  <q-page class="orderlist">
    ...
    >
      <q-card class="poptip">
        ...
        <q-card-section class="row justify-center">
          <span class="q-ml-sm">收货号:{{shipmentId}}</span>
...
export default {
  ...
  methods: {
    ...
    getorder () {
      this.$api.Rcv.GetOrderByDocumentOrderId(this.orderNo).then(res => {
        // 获取订单信息
        ...
        this.orderList = res.data.data.orderLines.map(val => {
          ...
          json.qtyReceived = val.qtyOrdered
          return json
        })
        this.rcvorder = this._.uniqBy(this.orderList, 'itemId')
        console.log('订单信息')
    ...
    computedRcv () {
      this.$api.Ship.CompleteFreshPoShipment(this.orderNo).then(res => {
        // 收货完成
