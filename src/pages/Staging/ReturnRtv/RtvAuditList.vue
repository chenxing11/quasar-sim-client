...
export default {
  ...
  watch: {
    tempSelected (val) {
      if (this.tempSelected[0]) {
        this.rtvId = this.tempSelected[0].id
        this.get_order(this.rtvId)
      }
    }
  },
  created () {
  ...
  methods: {
    ...
    get_order (id) {
      this.$router.push({ path: '/ReturnRtv/RtvAuditOrder', query: { rtvId: id } })
    },
    getList () { // 根据地点获取已提交的退货单
      this.$api.Rtv.ListReturnIntentionInSubmittedByLoc(this.LocationId).then(res => {
        console.log(res)
        if (res.data.code > 0) {
          this.temp = false
          this.rtvData = res.data.data.map(val => {
            let json = {}
            json.id = val.id
