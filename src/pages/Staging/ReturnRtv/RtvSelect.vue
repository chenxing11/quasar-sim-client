          ...
          <template v-slot:item="props">
            ...
            <q-card>
                <q-card-section horizontal>
                  <q-card-section
                    class="col"
                    @click="rtvOrder(props.row.id, props.row.createdDate)"
                  >
                  ...
                  <q-card-actions
                    class="col-2"
                  >
                    <span @click="dismantleBtn(props.row)">查看</span>
                  </q-card-actions>
                </q-card-section>
  ...
  watch: {
    tempSelected (val) {
      if (this.tempSelected[0]) {
        this.rtvOrder(this.tempSelected[0].id)
      }
    }
  },
  created () {
    this.CHANGE_TITLE('退单查询')
    ...
    searchBtn () {
      ...
      if (date > 7) {
        ...
        var data = {
          ...
          ReturnIntentionId: this.rtvId === '' ? null : this.rtvId,
          Status: ['A', 'S', 'W'],
        ...
        this.$api.Rtv.QueryReturnIntentions(data).then(res => {
          console.log(res)
          if (res.data.code > 0) {
            this.temp = false
            this.rtvData = res.data.data.returnIntentions.map(val => {
              let json = {}
              json.id = val.id
              json.createdDate = val.createdDate.slice(0, 10)
              json.status = this.formatStatus(val.status)
              json.returns = val.returns
              return json
            })
          } else {
            this.temp = true
      ...
      // } else {
      //   this.POP_MSG('退单编号或时间不能为空')
      // }
    },
    rtvOrder (id) {
      this.$router.push({ path: '/ReturnRtv/RtvOrder', query: { rtvId: id } })
      // A 已审批 S 已提交 W 工作表
    },
    dismantleBtn (row) {
      console.log(row)
      this.$router.push({ path: '/ReturnRtv/Dismantle', query: { data: JSON.stringify(row.returns) } })
    },
    formatStatus (val) {
      var text
      switch (val) {
