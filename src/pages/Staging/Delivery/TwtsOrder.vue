        ...
        <q-list>
          <q-item-section>
            ...
            >
              ...
              <span class="q-pa-sm text-grey-8">{{wtsOrder.length}}</span>
...
export default {
  ...
  methods: {
    ...
    comfirmShipment () { // 收货完成
      var json = this.$qs.stringify({ 'shipmentId': this.shipmentId })
      console.log(json)
      this.$api.Ship.CompleteWTSShipmentActualReceived(json).then(res => { // 配送单收货
        console.log(res)
        if (res.data.code === 1) {
