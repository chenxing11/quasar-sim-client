  ...
  data () {
    return {
      columns: [
        ...
        { name: 'uom', label: '配送数量', field: 'uom' },
        { name: 'qtyExpected', label: '箱数', field: 'qtyExpected' },
        { name: 'qtyReceived', label: '件数', field: 'qtyReceived' }
      ],
      orderList: [],
      ...
      this.$api.Rcv.getShipmentById(val).then(res => {
        ...
        if (res.data.code > 0) {
          this.orderList = res.data.data.shipmentLines.map((val, index) => {
            ...
            json.qtyExpected = parseInt(val.qtyReceived / val.qtyExpected)
